      ...
      <Container className="px-0">
        <article className="rounded-lg border border-[#30343B] bg-[#121418] p-6 md:p-8">
          <div className="flex flex-wrap gap-2">
            <Badge className="bg-[#F0B24C] text-[#08090B]">
              {guide.category}
            </Badge>
            <Badge
              variant="outline"
          ...
          <p className="mt-5 text-[#BDBAB4] text-lg leading-8">
            {guide.summary}
          </p>

          <div className="relative mt-8 aspect-video overflow-hidden rounded-lg border border-[#30343B]">
            <Image
          ...
          <div className="mt-8 space-y-8">
            {guide.body.map((section, index) => (
              <Fragment key={section.heading}>
                ...
                {guide.video && index === 1 ? (
                  <div className="overflow-hidden rounded-lg border border-[#30343B] bg-black">
                    <iframe
                      ...
                      allowFullScreen
                    />
                    <div className="bg-[#08090B] px-4 py-3 text-[#BDBAB4] text-sm">
                      Walkthrough cross-check:{' '}
                      <a
                        href={guide.video.url}
                        target="_blank"
                        rel="noreferrer"
                        className="text-[#E43D30] underline underline-offset-4"
                      >
                        {guide.video.title}
                      </a>{' '}
          ...
          <section className="mt-10 rounded-lg border border-[#30343B] bg-[#08090B] p-5">
            <h2 className="font-display text-[#FFE0A6] text-2xl font-bold">
              Related next steps
            </h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {guide.relatedRoutes.map((route) => (
                <LocaleLink
                  key={route}
                  href={route}
                  className="flex h-auto min-w-0 items-center justify-between gap-3 whitespace-normal rounded-md border border-[#30343B] bg-[#121418] px-4 py-3 text-left text-[#BDBAB4] text-sm transition hover:border-[#E43D30] hover:text-[#E43D30]"
                >
